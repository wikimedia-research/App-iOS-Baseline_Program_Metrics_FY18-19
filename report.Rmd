...
p <- ggplot2::ggplot(data = target_wiki_edit_by_editor_pdata, aes(x = edits, y = n_editor, fill = month)) +
...
p <- ggplot2::ggplot(data = target_content_edit_by_editor_pdata, aes(x = content_edits, y = n_editor, fill = month)) +
  ggplot2::geom_bar(stat = "identity", position = "dodge") +
  ggplot2::scale_fill_brewer("Month", palette = "Set1") +
...
```{r revert_rate_plot, fig.height=10}
revert_rates_pdata <- revert_rates %>%
  filter(`Number of edits` > 10) %>%
  arrange(`Revert rate`) %>%
  dplyr::mutate(Language=factor(Language, levels = Language))
ggplot(revert_rates_pdata, aes(x=Language, y=`Revert rate`)) +
  geom_segment( aes(x=Language, xend=Language, y=0, yend=`Revert rate` ), color=ifelse(revert_rates_pdata$wiki %in% target_wikis, "orange", "grey"), size=ifelse(revert_rates_pdata$wiki %in% target_wikis, 1.3, 0.7) ) +
  geom_point( color=ifelse(revert_rates_pdata$wiki %in% target_wikis, "orange", "grey"), size=ifelse(revert_rates_pdata$wiki %in% target_wikis, 5, 2) ) +
  scale_y_continuous(labels = scales::percent_format()) + 
  ggplot2::geom_text(aes(label = ifelse(revert_rates_pdata$wiki %in% target_wikis, scales::percent(revert_rates_pdata$`Revert rate`), ""), hjust = -0.5), size = 3, color = "orange") +
  theme_light() +
  coord_flip() +
  theme(
  ...
  labs(xlab="", ylab="Revert Rate", title = "Revert rate of iOS edits by Wikipedia, with target Wikipedias highlighted", subtitle = glue("Only include wikipedias with more than 10 edits from {start_date} to {end_date}"))
```
...
new_ios_editor_retention_summary <- new_ios_editor_retention %>%
  rename('Language'=language, 'Number of editors' = n_users, 
         'Median edits/editor' = median_edits_per_user, 
         'Total edits' = total_edits, '2nd-7th day retention' = retention_first6,
         '8th-14th day retention' = retention_7, '16th-30th day retention' = retention_15, 
         '31st-60th day retention' = retention_30)

sketch_new_ios_editor_retention_summary <- htmltools::withTags(table(
  DT::tableHeader(c("", names(new_ios_editor_retention_summary))),
  DT::tableFooter(c(rep("",2), names(new_ios_editor_retention_summary)[2], "", names(new_ios_editor_retention_summary)[4], rep("",4)))
))
DT::datatable(
...
DT::formatPercentage(c('2nd-7th day retention', '8th-14th day retention', '16th-30th day retention', '31st-60th day retention'), 2)
```
...
sketch_active_ios_editor_retention_summary <- htmltools::withTags(table(
  ...
  DT::tableFooter(c(rep("",2), names(active_ios_editor_retention_summary)[2], "", names(active_ios_editor_retention_summary)[4], rep("",4)))
))
DT::datatable(
